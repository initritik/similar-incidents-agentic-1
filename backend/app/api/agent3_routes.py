...
@router.post(
    ...
    summary="Process similar ticket retrieval and ranking",
    description=(
        "Run Agent 3 to filter, rank, and prepare similar tickets from Agent 2 results. "
        "Agent 3 filters tickets by similarity threshold (30%) and selects top 5."
    ),
)
def process_similar_incidents(agent2_results: dict) -> Agent3Response:
    """
    Process Agent 2 results to retrieve and rank similar tickets.

    Args:
        agent2_results: Response payload from Agent 2.

    Returns:
        Agent3Response with filtered top 5 tickets and future routing flag.
    """
